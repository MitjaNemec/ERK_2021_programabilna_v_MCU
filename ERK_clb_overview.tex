...
implementation is often demanding. In this article, we explore one of the
answers to this problem offered by the manufacturer Texas Instruments. In their
microcontrollers, there is an additional coprocessor, called ``CLB''. The
composition of said coprocessor is described, which is essential for the use of
such system.
\end{abstract}
...
tiskanega vezja okoli FPGA vezja zashteva več truda, dodaten trud pa zahteva
...
Ti signali lahko izvirajo iz različnih dodatkov, kot so eCAP, ePWM, GPIO... iz
CLB-ja pa lahko v prav te dodatke tudi ``pripeljemo'' izhodni signal, kot
njihov vhod.
...
% Prva slika je odstranjena, ker se mi zdi da nima neke prevelike vrednosti,
% in da se da dobro razumeti tudi brez slike.

% naslednja slika pa se zamenja z Figure 2-11 iz spraco2a
